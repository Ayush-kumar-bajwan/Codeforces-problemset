...
void solve()
{
   ...
   for(int j=0 ; j<4; j++){
        st1.insert(make_pair(k1 + dx[j] * a, k2 + dy[j] * b));
        st2.insert(make_pair(q1 + dx[j] * a, q2 + dy[j] * b));
        st1.insert(make_pair(k1 + dx[j] * b, k2 + dy[j] * a));
        st2.insert(make_pair(q1 + dx[j] * b, q2 + dy[j] * a));
    }
